...
def fetch_spot(spot: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    """Fetch astronomy once and each weather model separately.

    Separate model requests are more robust than one multi-model request because
    Open-Meteo variable availability and response field names can differ by model.
    """
    common = {
    ...
    for display_name, model_id in config["models"].items():
        model_payloads[display_name] = fetch_json({
            **common,
...
def calculate_models(data: dict[str, Any], config: dict[str, Any],
                     sunset: datetime) -> list[ModelResult]:
    results: list[ModelResult] = []

    for display, model_id in config["models"].items():
        payload = data["models"][display]
        ...
        vals = {
            var: avg([hourly[var][i] for i in idx])
        ...
        results.append(ModelResult(
            display,
            sunset_score(
                vals["cloud_cover_low"],
                vals["cloud_cover_mid"],
                vals["cloud_cover_high"],
                vals["precipitation"],
                vals["relative_humidity_2m"],
                vals["wind_speed_10m"],
            ),
            vals["cloud_cover_low"],
            vals["cloud_cover_mid"],
    ...
    for key, region in config["regions"].items():
        ...
        for spot in region["spots"]:
            try:
                ...
                print(f"WARNING {spot['name']}: {type(exc).__name__}: {exc}", flush=True)
